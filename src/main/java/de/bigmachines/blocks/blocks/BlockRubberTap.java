        				...
        				if(rubberIBlockState.getBlock() == ModBlocks.blockRubberLog) {
            				...
            				FluidActionResult result = FluidUtil.tryFillContainerAndStow(stack, new IFluidHandler() {
								...
								public FluidStack drain(int maxDrain, boolean doDrain) {
		            				...
		            				if(doDrain) worldIn.setBlockState(rubberPos, rubberIBlockState.withProperty(BlockRubberLog.RUBBER_LEVEL, Math.max(0, Math.min(3, rubberIBlockState.getValue(BlockRubberLog.RUBBER_LEVEL).intValue() - ((int)Math.ceil((double)fluidStack.amount / 500))))));
