	...
	public static class WorldGeneratorRubberTreeDeserializer implements JsonDeserializer<WorldGeneratorRubberTree> {
		...
		public WorldGeneratorRubberTree deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			if (jsonObj.has("blacklisted-dimensions") && jsonObj.get("blacklisted-dimensions").isJsonArray()) {
				blacklistedDimensions = WorldGenerationConfig.gson.fromJson(jsonObj.get("blacklisted-dimensions").getAsJsonArray(), new TypeToken<List<Integer>>() {
				}.getType());
			} else {
	...
	public static class Generator extends WorldGenAbstractTree {
		...
		public boolean generate(World worldIn, Random rand, BlockPos position) {
			if (checkForBlockFreedom(worldIn, position.add(0, 1, 0), position.add(0, 5, 0))) {
				...
				if (isReplaceable(worldIn, position.add(1, height - 1, 0)))
					worldIn.setBlockState(position.add(1, height - 1, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 1, 0)))
					worldIn.setBlockState(position.add(-1, height - 1, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 1, 1)))
					worldIn.setBlockState(position.add(0, height - 1, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 1, -1)))
					worldIn.setBlockState(position.add(0, height - 1, -1), blockStateLeaves);
				
				if (isReplaceable(worldIn, position.add(1, height - 2, 0)))
					worldIn.setBlockState(position.add(1, height - 2, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 2, 0)))
					worldIn.setBlockState(position.add(-1, height - 2, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 2, 1)))
					worldIn.setBlockState(position.add(0, height - 2, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 2, -1)))
					worldIn.setBlockState(position.add(0, height - 2, -1), blockStateLeaves);
				
				if (isReplaceable(worldIn, position.add(1, height - 3, 0)))
					worldIn.setBlockState(position.add(1, height - 3, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 3, 0)))
					worldIn.setBlockState(position.add(-1, height - 3, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 3, 1)))
					worldIn.setBlockState(position.add(0, height - 3, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 3, -1)))
					worldIn.setBlockState(position.add(0, height - 3, -1), blockStateLeaves);
				
				if (isReplaceable(worldIn, position.add(1, height - 3, 1)))
					worldIn.setBlockState(position.add(1, height - 3, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 3, 1)))
					worldIn.setBlockState(position.add(-1, height - 3, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 3, -1)))
					worldIn.setBlockState(position.add(-1, height - 3, -1), blockStateLeaves);
				...
				if (isReplaceable(worldIn, position.add(2, height - 3, 0)))
					worldIn.setBlockState(position.add(2, height - 3, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-2, height - 3, 0)))
					worldIn.setBlockState(position.add(-2, height - 3, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 3, 2)))
					worldIn.setBlockState(position.add(0, height - 3, 2), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 3, -2)))
					worldIn.setBlockState(position.add(0, height - 3, -2), blockStateLeaves);
				
				if (isReplaceable(worldIn, position.add(1, height - 4, 0)))
					worldIn.setBlockState(position.add(1, height - 4, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 4, 0)))
					worldIn.setBlockState(position.add(-1, height - 4, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 4, 1)))
					worldIn.setBlockState(position.add(0, height - 4, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 4, -1)))
					worldIn.setBlockState(position.add(0, height - 4, -1), blockStateLeaves);
				
				if (isReplaceable(worldIn, position.add(1, height - 4, 1)))
					worldIn.setBlockState(position.add(1, height - 4, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 4, 1)))
					worldIn.setBlockState(position.add(-1, height - 4, 1), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-1, height - 4, -1)))
					worldIn.setBlockState(position.add(-1, height - 4, -1), blockStateLeaves);
				...
				if (isReplaceable(worldIn, position.add(2, height - 4, 0)))
					worldIn.setBlockState(position.add(2, height - 4, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(-2, height - 4, 0)))
					worldIn.setBlockState(position.add(-2, height - 4, 0), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 4, 2)))
					worldIn.setBlockState(position.add(0, height - 4, 2), blockStateLeaves);
				if (isReplaceable(worldIn, position.add(0, height - 4, -2)))
					worldIn.setBlockState(position.add(0, height - 4, -2), blockStateLeaves);
